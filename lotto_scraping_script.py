...
# #### list all files in the raw html directory to extract
def list_files(path):
    ls = os.listdir(path)
...
def extract_html_list_content(HTML_path, year):
    ...
    with open (HTML_path + year + "-wyniki.html", "r", encoding="utf-8") as file:
        content = file.read()
...
# download html pages
if ! os.path.isdir(HTML_path) or ! len(list_files(HTML_path)):
...
print(len(whole_results_list))
print(len(whole_results_dict))
